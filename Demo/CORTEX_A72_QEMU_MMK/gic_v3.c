...
void gic_v3_initialize(void)
{
    ...
    vSendString("gic_v3_initialize()\n");
	init_gicd();
	init_gicc();
	gicd_config(TIMER0_IRQ, GIC_GICD_ICFGR_EDGE);
	gicd_set_priority(TIMER0_IRQ, 0 << GIC_PRI_SHIFT );  /* Set priority */
	gicd_set_target(TIMER0_IRQ, 0x1);  /* processor 0 */
	gicd_clear_pending(TIMER0_IRQ);
	gicd_enable_int(TIMER0_IRQ);
}
 ...
 */
int gic_v3_find_pending_irq(uint64_t *context __attribute__((unused)), irq_no *irqp) {
	int   rc;
	irq_no i;
...
void gic_v3_handler(void) {
	int irq;
	__asm__ volatile("MOV %0, sp":"=r"(irq));
	vSendStringISR("sp=");
	printHexISR(irq);

	extern void _stack_bottom(void);
	printHexISR(&_stack_bottom);
	configASSERT(irq > _stack_bottom);

	gic_v3_find_pending_irq(NULL, &irq);
	vSendStringISR(" gic_v3_handle\n");
	if(irq == 27) {
		gic_v3_eoi(irq);
		FreeRTOS_Tick_Handler();
		//FreeRTOS_IRQ_Handler();
	}
	if(irq > configMAX_API_CALL_INTERRUPT_PRIORITY){
		FreeRTOS_IRQ_Handler();
	}else{
		portDISABLE_INTERRUPTS();
		vSendStringISR("gic_v3 handle CRITICAL: ");
    	printHex(irq);
	}

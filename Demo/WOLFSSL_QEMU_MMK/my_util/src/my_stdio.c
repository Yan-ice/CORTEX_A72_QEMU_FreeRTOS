...
void intToHex(unsigned long long num, char *hexStr) {
    // 十六进制字符表
...
int setup_stdio(){
    // 使用 fdev_setup_stream 初始化 uart_stream
    fdev_setup_stream(&stdout_file, std_putchar, std_getchar, std_flush, _FDEV_SETUP_WRITE);
    fdev_setup_stream(&stderr_file, std_putchar, std_getchar, std_flush, _FDEV_SETUP_WRITE);
    return 0;
}
